...
//     helper_executable: which (and where) is the EXE that will write the
//                        pipe's output
///////////////////////////////////////////////////////////////////////////////
                            ...
                            const char *helper_executable/*=NULL*/)
{
    ...
    // Extra console : create another process , it's role is to display the 
    //     pipe's output
    STARTUPINFO si;
    ...
    BOOL bRet = CreateProcess(NULL, cmdline, NULL, NULL, FALSE,
                              CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
    if (!bRet) // on failure - try to get the path from the environment
    {
    ...
    ///////////////////////////////////////////////////////////////////
    // In order to easily add new future-features , i've chosen to pass
    // the "extra" parameters just the HTTP protocol - via textual 
    // "headers" .
    ...
    if (buffer_size_x != -1 && buffer_size_y != -1)
    {
        ...
        if (cbWritten != strlen(buffer))
        {
            MessageBox(NULL, "WriteFile failed(2)",
                       "ConsoleLogger failed", MB_OK);
            DisconnectNamedPipe(m_hPipe);
            CloseHandle(m_hPipe);
            m_hPipe = INVALID_HANDLE_VALUE;
            return -1;
...
{
    DWORD dwWritten = (DWORD) -1;
    // we assume that in iSize < 2^24 , because we're using only 3 bytes of
    //     iSize
    // 32BIT: send DWORD = 4bytes: one byte is the command (COMMAND_PRINT), 
    //     and 3 bytes for size
    DWORD command_plus_size = (COMMAND_PRINT << 24) | iSize;
    EnterCriticalSection();
    if ( !WriteFile (m_hPipe, &command_plus_size, sizeof(DWORD),
                     &dwWritten, NULL)
...
int CConsoleLoggerEx::_cprint(int attributes, const char *lpszText, int iSize)
{
    ...
    // we assume that in iSize < 2^24 , because we're using only 3 bytes of
    //     iSize
    // 32BIT: send DWORD = 4bytes: one byte is the command (COMMAND_CPRINT),
    //     and 3 bytes for size
